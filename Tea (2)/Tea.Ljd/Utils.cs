...
namespace Tea.Common
{
    public class Utils
    {
        ...
        {
            return TWKeys.ASSEMBLY_VERSION;
        }
        #endregion
        ...
        public static string SaveFile(string str, string fullPath)
        {
        ...
        /// <param name="pageIndex">当前頁</param>
        /// <param name="totalCount">总記錄数</param>
        /// <param name="linkUrl">链接地址，__id__代表頁码</param>
        /// <param name="centSize">中间頁码数量</param>
        /// <returns></returns>
        public static string OutPageList(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            ...
            int firstNum = pageIndex - (centSize / 2); //中间开始的頁码
            if (pageIndex < centSize)
                firstNum = 2;
            int lastNum = pageIndex + centSize - ((centSize / 2) + 1); //中间结束的頁码
        ...
        /// <param name="centSize">中间頁码数量</param>
        /// <returns></returns>
        public static string OutPageListWeb(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            //计算頁数
            if (totalCount < 1 || pageSize < 1)
            {
            ...
            if (pageIndex == 1)
            {
                firstStr = "<a href=\"" + ReplaceStr(linkUrl, pageId,"1") + "\" class=\"active\">1</a>";
            }
            if (pageIndex == pageCount)
            {
                lastStr = "<a href=\"" + ReplaceStr(linkUrl, pageId, pageCount.ToString()) + "\" class=\"active\">" + pageCount.ToString() + "</a>";
            }
            int firstNum = pageIndex - (centSize / 2); //中间开始的頁码
            if (pageIndex < centSize)
                firstNum = 2;
            ...
            for (int i = firstNum; i <= lastNum; i++)
            {
                if (i == pageIndex)
                {
                    pageStr.Append("<a href=\"" + ReplaceStr(linkUrl, pageId, i.ToString()) + "\" class=\"active\">" + i + "</a>");
                }
                else
                {
                    pageStr.Append("<a href=\"" + ReplaceStr(linkUrl, pageId, i.ToString()) + "\">" + i + "</a>");
            ...
            pageStr.Append(lastStr + lastBtn);
            //pageStr.Append("<em>共" + totalCount + "記錄</em>");
            return pageStr.ToString();
        }
        #endregion
        ...
        public static int DateDiff(System.DateTime Date1, System.DateTime Date2, string Interval)
        {
            ...
            {
                case "y"://返回日期的年份间隔   
                    are = System.Convert.ToInt32(objT.Days / dblYearLen);
                    break;
                case "M"://返回日期的月份间隔   
                    are = System.Convert.ToInt32(objT.Days / dblMonthLen);
                    break;
                case "d"://返回日期的天数间隔   
                    are = objT.Days + 1;
                    break;
                case "h"://返回日期的小时间隔   
                    are = System.Convert.ToInt32(objT.TotalHours);
                    break;
                case "m"://返回日期的分钟间隔   
                    are = System.Convert.ToInt32(objT.TotalMinutes);
                    break;
                case "s"://返回日期的秒钟间隔   
                    are = System.Convert.ToInt32(objT.TotalSeconds);
                    break;
                case "ms"://返回时间的微秒间隔   
                    are = System.Convert.ToInt32(objT.TotalMilliseconds);
                    break;
                default:
                    break;
            }
        ...
        public static int Date_Diff(System.DateTime Date1, string Date2 , string Interval)
        {
            double dblYearLen = 365;//年的长度，365天   
            double dblMonthLen = (365 / 12);//每个月平均的天数   
            System.TimeSpan objT;
           
            objT =  Utils.StrToDateTime(Date2).Subtract(Date1);
            int are = 0;
            switch (Interval)
            {
                case "y"://返回日期的年份间隔   
