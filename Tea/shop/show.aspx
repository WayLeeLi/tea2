...
<body>
    ...
    <div class="width-sys location">
        <a href="/">首頁</a> &gt; <%=tname%> &gt; <%=model.title%></div>
    <div class="width-sys">
        <div class="sub-left">
            <div class="pro-lb">
                商品類別</div>
...
<div class="detail_pic" id="detail-p">
  ...
  <div class="jqueryzoom">
   ...
   <div id="detail_pic" class="owl-carousel">
     <div class="item">
		<img class="cloudzoom-gallery" src="<%=model.img_url%>" data-cloudzoom="useZoom:'.cloudzoom',image:'<%=model.img_url%>',zoomImage:'<%=model.img_url%>'"/> 
    </div>
		...
		<img class="cloudzoom-gallery" src="<%#Eval("original_path")%>" data-cloudzoom="useZoom:'.cloudzoom',image:'<%#Eval("original_path")%>',zoomImage:'<%#Eval("original_path")%>'"/> 
            ...
            <div class="detail-l wap-show">
                <div id="detail" class="slider-pro">
                    <div class="sp-slides">
                       <div class="sp-slide">
                            <img class="sp-image" src="/css/images/blank.gif" data-src="<%=model.img_url%>" data-retina="<%=model.img_url%>" />
                        </div>
            ...
            <div class="detail-r">
                <div class="iteminfo_buying">
                    ...
                    <div class="sys_item_spec">
                        <dl class="clearfix iteminfo_parameter lh32">
                            <dd><span class="float-r share-ico"><a href="javascript:window.open('http://www.facebook.com/sharer.php?u='+encodeURIComponent(document.location.href)+'&t='+encodeURIComponent(document.title));void(0)" title="分享到Facebook"><img src="/images/Facebook.png" alt="分享到Facebook" width="29" height="30" /><a href="javascript:;" onclick="TraceAdd(this, '/', 0, '<%=id%>')" title="加入最愛"><img src="/images/xinx.png" width="29" height="30" alt="加入最愛" /></a></span><span class="all_price">NT$ <b class="sys_item_mktprice"><%=model.market_price.ToString("0.")%></b></span><span class="iteminfo_price"> <b>NT$</b><b class="sys_item_price"> <%=model.sell_price.ToString("0.")%></b></span></dd>
                        </dl>
                    </div>
                        ...
                        <dl class="clearfix iteminfo_parameter sys_item_specpara" data-sid="2">
                            <dd>
                                <ul class="sys_spec_text">
                                    ...
                                    <li data-cloudzoom="useZoom:'.cloudzoom',image:'<%#Eval("img_url")%>',zoomImage:'<%#Eval("img_url")%>'"  data-aid="<%#Eval("parent_id")%>_<%#Eval("id")%>" <%#Container.ItemIndex==0?" class=\"cloudzoom-gallery selected\"":" class=\"cloudzoom-gallery\""%> ><a href="javascript:;"title="<%#Eval("guige")%>" ><%#Eval("guige")%></a><i></i></li>
                                    </ItemTemplate></asp:Repeater>
                                </ul>
                            </dd>
                        </dl>
                        <dl class="clearfix iteminfo_parameter lh32">
                            <dd>
                                <div class="float-l"><form id="carform" action="cart.aspx" method="get"><input type="hidden" id="cartid" name="id" value="<%=cartid%>" />
                                    ...
                                    <span class="kucun" id="kc" <%if(stock_quantity>config.txt_Shen){%>style="display:none;"<%} %>>還剩<span id="ku"><%=stock_quantity%></span>件</span></form>
                                </div>
                            </dd>
                        </dl>
                        <div class="hr">
                ...
                <script>
                    //价格json
                    var sys_item = {
                        "mktprice": "<%=model.market_price.ToString("0.")%>",
                        "price": "<%=model.sell_price.ToString("0.")%>",
                        "cartid": "<%=cartid%>",
...
//                                    i.removeAttr("data-attrval");
                ...
                <div class="detail-btn" id="gwc">
                    ...
                    <a href="javascript:;" onClick="CartAdd(this, '/', 0, '/shop/show.aspx?id=<%=model.id%>',<%=config.txt_Da%>);" class="cart-btn">加入購物車</a> <a href="javascript:;" onclick="javascript:$('#carform').submit();" class="Collection-btn">直接結帳</a>
                </div>
                ...
                <%if (model.brand_id == 2)
                  { %>
                預計出貨日期：<span class="detail-tips red"><%=model.update_time.GetValueOrDefault().ToString("yyyy-MM-dd")%></span>
                <%} %>
                 <%if (!string.IsNullOrEmpty(model.zhuyi))
                  { %>
             <%--   預計到貨：<span class="detail-tips red"><%=model.zhuyi.Replace("|"," ")%></span>--%>
                <%} %>
                <%if(model.brand_id==5 && model.begin_time!=model.end_time){ %>
                 促銷日期：<span class="detail-tips red"><%=model.begin_time.ToString("yyyy-MM-dd")%>~<%=model.end_time.ToString("yyyy-MM-dd") %></span>
                <%} %>
            </div>
            <div class="clear">
            </div>
            <div id="Tabs1">
                <ul>
                    <li><a href="#tabs-1" <%=gettag(model.tags,0).ToString().Length>1?"":" style=\"display:none;\"" %>><%=gettag(model.tags,0)%></a></li>
                    <li><a href="#tabs-2" <%=gettag(model.tags,1).ToString().Length>1?"":" style=\"display:none;\"" %>><%=gettag(model.tags,1)%></a></li>
                    <li><a href="#tabs-3" <%=gettag(model.tags,2).ToString().Length>1?"":" style=\"display:none;\"" %>><%=gettag(model.tags,2)%></a></li>
                </ul>
                <div id="tabs-1" <%=gettag(model.tags,0).ToString().Length>1?"":" style=\"display:none;\"" %>>
                    <div class="detail-detail">
                        <%=model.content%>
                    </div>
                </div>
                <div id="tabs-2" <%=gettag(model.tags,1).ToString().Length>1?"":" style=\"display:none;\"" %>>
                    <div class="detail-detail">
                        <%=model.guigemore%>
                    </div>
                </div>
                <div id="tabs-3" <%=gettag(model.tags,2).ToString().Length>1?"":" style=\"display:none;\"" %>>
                    <div class="detail-detail">
                         <%=model.shuoming%>
                    </div>
                </div>
