...
public partial class index : Tea.Web.UI.ShopPage
{
    ...
    protected string sqlkey = "channel_id=7 and (datediff(day,xia_date,getdate())<=0 or xia_date is null) and status=0 and datediff(minute,add_time,getdate())>=0 and brand_id in(1,2,3,5)", _sql = "channel_id=7 and (datediff(day,xia_date,getdate())<=0 or xia_date is null) and status=0 and datediff(minute,add_time,getdate())>=0 and brand_id in(1,2,3,5)", _orderby = "add_time desc,id desc", key, tname, sql;
    protected int tid;
    protected int hid;
    protected int sort;
