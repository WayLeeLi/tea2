...
<body>
    ...
    <div class="width-sys location">
        <a href="/">首頁</a> &gt; <a href="/shop/users.aspx">會員專區</a></div>
    <div class="width-sys">
        <div class="sub-left">
        ...
        <div class="sub-right">
            <div class="sub-search">
                <div class="sub-search-sx">
                    <a href="users.aspx?tid=<%=tid%>&key=<%=key%>&sort=1">最新上架</a></div>
                <div class="sub-search-sx">
                    <a href="users.aspx?tid=<%=tid%>&key=<%=key%>&sort=2">熱門排行</a></div>
                <div class="sub-search-sx">
                    <a href="users.aspx?tid=<%=tid%>&key=<%=key%>&sort=3">價格由低至高</a></div>
                <div class="sub-search-sx">
                    <a href="users.aspx?tid=<%=tid%>&key=<%=key%>&sort=4">價格由高至低</a></div>
            </div>
            <div class="list-four-sub">
                <ul>
                    <asp:Repeater ID="data_list" runat="server"><ItemTemplate>
                    <li><a href="/shop/show.aspx?id=<%#Eval("aid")%>&gid=<%#Eval("id")%>">
                        <img src="<%#Eval("img_url")%>" alt="<%#Eval("title")%>" /></a>
                        <div class="list-title">
                            <a href="/shop/show.aspx?id=<%#Eval("aid")%>&gid=<%#Eval("id")%>"><%#Eval("title")%></a></div>
                        <div class="list-ps"> <span class="price">NT$<%#Eval("price","{0:0.}")%></span>
                        </div>
                    </li>
