...
<p>您已經成功完成交易！</p>
   ...
   <p class="tit">以下是您本次的交易明細</p>
      ...
      <p>交易方式：<span class="fs"> <%=new Tea.BLL.payment().GetTitle(model.payment_id)%></span></p>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td align="center">
      <div class="price-box1" <%#getpoint(Eval("goods_where").ToString(),1)%>> <span class="wap-show" ></span><span class="price">NT$<%#Tea.Common.Utils.StrToInt(Eval("quantity").ToString(), 0) * Tea.Common.Utils.StrToInt(Eval("real_price", "{0:0.}").ToString(), 0)%></span></div>
                                    <div class="price-box1" <%#getpoint(Eval("goods_where").ToString(),2)%>> <span class="wap-show"></span><span class="price"><%#Tea.Common.Utils.StrToInt(Eval("quantity").ToString(), 0) * Tea.Common.Utils.StrToInt(Eval("point", "{0:0.}").ToString(), 0)%></span>點</div>
    ...
    <tr>
      <td width="60" align="center"><img src="<%=weburl%><%#Eval("img_url")%>" width="46" height="37" alt=""/></td>
      <td align="left"><%#Eval("title")%></td>
      <td align="center"><%#Eval("ocompany")%></td>
      <td align="center"></td>
      <td align="center"></td>
    </tr>
...
<p class="tit">送貨資料</p>
      ...
      <p>聯絡電話：<span class="fs">  <%=getuseradd(model.user_add,2)%>  <%=getuseradd(model.user_add,3).Replace(",","")%></span></p>
      ...
      <p>收件地址：<span class="fs"><%=getuseradd(model.user_add,8)%></span></p>
        ...
        <p>交易資料</p>
          <p>符合活動<span class="fs"> <%=model.zhe_else%></span></p>
            
       
         <p>合計<span class="fs"> NT$<%=model.real_amount.ToString("0.")%></span></p>
  
          <p>紅利點數兌商品<span class="fs"> <%=model.point%>點</span></p>
          <p>折扣<span class="fs"> $<%=model.zhe%></span></p>
    
          <p>運費<span class="fs"> NT$<%=model.express_fee.ToString("0.")%></span></p>
           <p>紅利折扣<span class="fs"> $<%=model.tuid%></span></p>
       
          <p>優惠券<span class="fs"> $<%=model.payment_fee.ToString("0.")%></span></p>
         <p>總金額（含運費）<span class="fs"> NT$<%=model.order_amount.ToString("0.")%></span></p>
         
         <p>本次消費共折抵紅利<span class="fs"> <%=model.company+model.point%>點</span></p>
       <p>本次消費共得紅利<span class="fs"> <%=model.num%>點</span></p>
