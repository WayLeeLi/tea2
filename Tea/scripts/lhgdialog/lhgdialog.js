...
/*!
 * _path 獲取組件核心文件lhgdialog.js所在的絕對路徑
 * _args 獲取lhgdialog.js文件後的url參數組，如：lhgdialog.js?self=true&skin=aero中的?後面的內容
 */
...
/*! 獲取 lhgdialog 可跨級調用的最高層的 window 對象和 document 對象 */
_doc, _top = (function(w)
{
	try{
	    _doc = w['top'].document;  // 跨域|無權限
		_doc.getElementsByTagName; // chrome 瀏覽器本地安全限制
	}catch(e){
	    _doc = w.document; return w;
	};
	
	// 如果指定參數self為true則不跨框架彈出，或為框架組則無法顯示協力廠商元素
...
/*!
 * IE6下Fixed無抖動靜止定位
 * 如果你的頁面的html元素設定了背景圖片請把設置背景圖片的css寫到body元素上
 * 如果你不需要組件靜止定位（也就是隨屏滾動）或此段代碼影響了你的頁面佈局可將此段代碼刪除
 */
_ie6 && (function(bg){
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		DOM.wrap.addClass( config.skin ); // 多皮膚共存
		...
		// 假如加載的是單獨頁面的內容頁config.init函數會在內容頁加載完成後執行，這裡就不執行了
	...
	/*!
	 * 設置內容
	 * @param	{String}	內容 (如果內容前3個字元為‘url:’就加載單獨頁面的內容頁)
	 * @param   {Boolean}   是否為後增加的內容
	 * @param   {Boolean}   是否使用iframe方式加載內容頁
	 * @return	{this}		如果無參數則返回對象本身
	 */
	content: function( msg, add, frm )
	{
		...
		var that = this, DOM = that.DOM,
		    ...
		    // 假如內容中前3個字元為'url:'就加載相對路徑的單獨頁面的內容頁
		if( frm )
		{
	...
	zindex: function()
	{
		...
		// 擴展視窗置頂功能，只用在iframe方式加載內容
		// 或跨域加載內容頁時點視窗內容主體部分置頂視窗
	...
	close: function()
	{
		...
		// 當使用iframe方式加載內容頁時的處理代碼
		if( that.iframe )
		{
			if( typeof  fn === 'function' && fn.call(that, that.iframe.contentWindow, window) === false )
			    return that;
			
			// 重要！需要重置iframe位元址，否則下次出現的對話框在IE6、7無法聚焦input
	...
	/*!
	 * 獲取指定id的視窗對象或視窗中iframe加載的內容頁的window對象
	 * @param {String} 指定的id
	 * @param {String} 是否返回的為指定id的視窗對象
	...
	_iframe: function( url )
	{
		...
		// 延遲加載iframe的src屬性，IE6下不延遲加載會出現加載進度條的BUG
		...
		var load = that._fmLoad = function()
		{
			...
			// 增強視窗置頂功能，iframe方式加載內容或跨域加載內容頁時點視窗內容部分置頂視窗
			// 通過使用重置loading層來優雅的完成此功能，在focus方法中有此功能的相關代碼
			...
			// 此部分代碼結束，在拖動改變大小的_dragEvent.onmove方法中還有此功能的相關代碼
			
			try{
			    iwin = that.content = iframe.contentWindow; // 定義窗口對象content屬性為內容頁的window對象
				$idoc = $(iwin.document);
			...
			// 視窗最大化時這裡不用再計算視窗的尺寸和位置了，如果再計算視窗會出現錯位元
			if( !that._maxState )
			{
		...
		// 綁定iframe元素api屬性為視窗自身對象，在內容頁中此屬性很重要
	...
	_reset: function()
	{
		...
		//IE6~8會出現最大化還原後視窗重新定位，鎖定滾動條在IE下就會觸發resize事件BUG 
...
/*!
 * 全域快捷鍵
 * 由於跨框架時事件是綁定到最頂層頁面，所以當當前頁面卸載時必須要除移此事件
 * 所以必須unbind此事件綁定的函數，所以這裡要給綁定的事件定義個函數
 * 這樣在當前頁面卸載時就可以移此事件綁定的相應函數，不而不影響頂層頁面此事件綁定的其它函數
 */
...
lhgdialog.setting =
{
	...
	max: true,                  // 是否顯示最大化按鈕
	min: true,                  // 是否顯示最小化按鈕
	zIndex: 1976,				// 對話框疊加高度值(重要：此值不能超過瀏覽器最大限制)
	resize: true,				// 是否允許用戶調節尺寸
	drag: true, 				// 是否允許用戶拖動位置
	cache: true,                // 是否緩存窗口內容頁
	data: null,                 // 傳遞各種數據
...
$(function(){
	...
	// 增強lhgdialog拖拽體驗（可選外置模塊，如不需要可刪除）
	// 防止鼠標落入iframe導致不流暢，對超大對話框拖動優化
 ...
 * @param	{String}	提示圖標 (注意要加擴展名)
 * @param   {Function}  提示關閉時執行的回調函數
 */
