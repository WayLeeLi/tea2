...
namespace Tea.Web.admin.settings
{
    public partial class sys_config : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                //ChkAdminLevel("sys_config", TWEnums.ActionEnum.View.ToString()); //檢查許可權
                ShowInfo();
            }
        }
        ...
        private void ShowInfo()
        {
            ...
            txt_Da.Text = model.txt_Da.ToString();
            txt_Shen.Text = model.txt_Shen.ToString();
            ...
            commentstatus.Text = model.commentstatus.ToString();
        ...
        {
            ChkAdminLevelEdit("sys_config", "Edit");
            //ChkAdminLevel("sys_config", TWEnums.ActionEnum.Edit.ToString()); //檢查許可權
            BLL.siteconfig bll = new BLL.siteconfig();
            Model.siteconfig model = bll.loadConfig();
            try
            {
                ...
                model.commentstatus = Utils.StrToInt(commentstatus.Text, 0);
                ...
                model.txt_Da = Utils.StrToInt(txt_Da.Text, 0);
                model.txt_Shen = Utils.StrToInt(txt_Shen.Text, 0);
                bll.saveConifg(model);
                AddAdminLog(TWEnums.ActionEnum.Edit.ToString(), "修改系統組態資訊"); //記錄日誌
                JscriptMsg("修改系統組態成功！", "sys_config.aspx");
            }
            catch
            {
                JscriptMsg("檔寫入失敗，請檢查資料夾許可權！", "");
            }
        }
