    ...
    public partial class edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            if (a > 0)
            {
                ...
                bll.Update(model1);
                AddAdminLog(TWEnums.ActionEnum.Add.ToString(), "添加分區設定：" + model.basic_label); //記錄日誌
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(TWEnums.ActionEnum.Edit.ToString(), "修改分區設定：" + model.basic_label); //記錄日誌
                result = true;
            }
