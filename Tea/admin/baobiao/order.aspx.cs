    ...
    public partial class order_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                RptBind("status=3" + CombSqlTxt(this.status, this.payment_status, this.express_status, this.keywords, data, begin, end, peisong), "id desc");
            }
        }
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            this.rptList.DataSource = bll_view.GetViewList("view_order", "", this.pageSize, this.page, _strWhere, _orderby, out this.totalCount);
            ...
            string pageUrl = Utils.CombUrlTxt("order.aspx", "status={0}&payment_status={1}&express_status={2}&keywords={3}&page={4}",
        ...
        protected string CombSqlTxt(int _status, int _payment_status, int _express_status, string _keywords, string _date, string _begin, string _end, int _peisong)
        {
            ...
            if (!string.IsNullOrEmpty(_begin) && !string.IsNullOrEmpty(_end))
            {
                ...
                strTemp.Append(" and (complete_time between '" + _begin + "' and '" + eend + "')");
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("order.aspx", "keywords={0}&begin={1}&end={2}", txtKeywords.Text, txtbegin.Text, txtend.Text));
        }
        ...
        public static string getcart(string cid)
        {
            ...
            try
            {
                str = Tea.DBUtility.DbHelperSQL.Query("select sum(quantity) from dt_order_goods where order_id=" + cid + "").Tables[0].Rows[0][0].ToString();
            }
            catch (Exception eee) { }
            return str;
        }
        ...
        {
            ChkAdminLevelEdit("baobiao_1", "Edit");
            string fileName = "訂單" + DateTime.Now.ToString("yyyyMMddHHmmssffff") + ".xls";
            Tea.Web.UI.ShopPage bll_view = new Tea.Web.UI.ShopPage();

            DataTable dt = bll_view.GetViewList("view_order", "", 10000, this.page, "status=3 " + CombSqlTxt(this.status, this.payment_status, this.express_status, this.keywords, data, begin, end, peisong), "order_id desc", out this.totalCount).Tables[0];

            string[] titleCol = new string[] { "購買日期", "訂單號碼", "會員", "購買人","商品編號", "商品名稱", "數量", "單價", "銷售金額" };
                ...
                for (i = 0; i < titleCol.Length; i++)
                {
                    switch (i)
                    {
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            sb.Append("<td>" + dr["goods_code"].ToString() + "</td>");
                            break;
                        case 6:
                        ...
                        case 7:
                            ...
                            break;
                        case 8:
                            sb.Append("<td>" + int.Parse(dr["quantity"].ToString()) * decimal.Parse(dr["real_price"].ToString()) + "</td>");
                            break;
                    }
                }
                sb.Append("</tr>");
        ...
        public string getname(string id)
        {
            ...
            try
            {
                str = new Tea.BLL.users().GetModel(int.Parse(id)).nick_name;
            }
            catch (Exception eee) { }
            return str;
        }
        ...
        public string getstate1(DateTime sid)
        {
            ...
            try
            {
                str = Tea.DBUtility.DbHelperSQL.Query("select sum(real_amount) from shop_orders where datediff(day,complete_time,'" + sid + "')=0").Tables[0].Rows[0][0].ToString();
            }
            catch (Exception eee) { }
            return str;
        }
