...
namespace Tea.Web.admin.about
{
    public partial class article_list : Web.UI.ManagePage
    {
        ...
        {
            this.channel_id = TWRequest.GetQueryInt("channel_id",2);
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("about_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("about_page_size", _pagesize.ToString(), 43200);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("list.aspx", "channel_id={0}&category_id={1}&keywords={2}&property={3}&status={4}",
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(TWEnums.ActionEnum.Edit.ToString(), "刪除" + this.channel_name + "頻道內容成功" + sucCount + "條，失敗" + errorCount + "條"); //記入日誌
