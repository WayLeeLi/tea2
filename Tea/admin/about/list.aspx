...
<div id="floatHead" class="toolbar-wrap">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
        <li style="display:none;"><a class="add" href="edit.aspx?action=<%=TWEnums.ActionEnum.Add %>&channel_id=<%=this.channel_id %>"><i></i><span>新增</span></a></li>
      ...
      <div class="menu-list">
        ...
        <div class="rule-single-select" id="statusSelect" runat="server" style="display:none;">
          <asp:DropDownList ID="ddlStatus" runat="server" AutoPostBack="True" onselectedindexchanged="ddlStatus_SelectedIndexChanged">
            <asp:ListItem Value="" Selected="True">所有</asp:ListItem>
            <asp:ListItem Value="1">上架</asp:ListItem>
            <asp:ListItem Value="3">下架</asp:ListItem>
            <asp:ListItem Value="2">審核</asp:ListItem>
          </asp:DropDownList>
        </div>
      </div>
    </div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
  <tr>
    <th width="3%"></th>
    <th align="left">標題</th>
    <th align="left" width="12%" style="display:none;">所屬類別</th>
    <th align="left" width="16%">發佈時間</th>
    <th align="left" width="65" style="display:none;">排序</th>
    <th align="left" width="110" style="display:none;">屬性</th>
    <th width="8%">操作</th>
  </tr>
</HeaderTemplate>
<ItemTemplate>
  <tr>
    <td align="center"><asp:CheckBox ID="chkId" Visible="false" CssClass="checkall" runat="server" style="vertical-align:middle;" /><asp:HiddenField ID="hidId" Value='<%#Eval("id")%>' runat="server" /></td>
    <td><a href="edit.aspx?action=<%#TWEnums.ActionEnum.Edit%>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>&url=page|<%=page%>-category_id|<%=category_id%>"><%#Eval("title")%></a></td>
    <td style="display:none;"><%#new Tea.BLL.article_category().GetTitle(Convert.ToInt32(Eval("category_id")))%></td>
    <td><%#Eval("add_time","{0:yyyy-MM-dd}")%></td>
    <td style="display:none;"><asp:TextBox ID="txtSortId" runat="server" Text='<%#Eval("sort_id")%>' CssClass="sort" onkeydown="return checkNumber(event);" /></td>
    <td style="display:none;">
      <div class="btn-tools">
        <asp:LinkButton ID="lbtnIsMsg" CommandName="lbtnIsMsg" runat="server" CssClass='<%# Convert.ToInt32(Eval("is_msg")) == 1 ? "msg selected" : "msg"%>' ToolTip='<%# Convert.ToInt32(Eval("is_msg")) == 1 ? "取消評論" : "設定評論"%>' />
       
        <asp:LinkButton ID="lbtnIsSlide" CommandName="lbtnIsSlide" runat="server" CssClass='<%# Convert.ToInt32(Eval("is_slide")) == 1 ? "pic selected" : "pic"%>' ToolTip='<%# Convert.ToInt32(Eval("is_slide")) == 1 ? "取消幻燈片" : "設定幻燈片"%>' />
      </div>
    </td>
