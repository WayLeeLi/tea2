<%@ Page Language="C#" AutoEventWireup="true" CodeFile="tuan_list.aspx.cs" Inherits="Tea.Web.admin.product.list" %>
...
<div id="floatHead" class="toolbar-wrap">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <li><a class="add" href="tuan_edit.aspx?action=<%=TWEnums.ActionEnum.Add %>&channel_id=<%=this.channel_id %>"><i></i><span>新增</span></a></li>
...
<div class="table-container">
  ...
  <ItemTemplate>
    <tr>
      ...
      <td><a href="tuan_edit.aspx?action=<%#TWEnums.ActionEnum.Edit%>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>&url=page|<%=page%>-category_id|<%=category_id%>-status|<%=status%>-keywords|<%=keywords%>"><%#Eval("title")%></a>(<a href="/shop/show.aspx?id=<%#Eval("id")%>" target="_blank">預覽</a>)</td>
      ...
      <td align="center">
        <a href="tuan_edit.aspx?action=<%#TWEnums.ActionEnum.Edit%>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>&url=page|<%=page%>-category_id|<%=category_id%>-status|<%=status%>-keywords|<%=keywords%>">修改</a> <asp:LinkButton ID="lbtnCopy" CommandName="lbtnCopy" runat="server" Text="複製"  style="display:none;" />
      </td>
    </tr>
  </ItemTemplate>
  <FooterTemplate>
  ...
  <ItemTemplate>
      <li>
        <div class="details<%#Eval("img_url").ToString() != "" ? "" : " nopic"%>">
          ...
          <h1><span><a href="tuan_edit.aspx?action=<%#TWEnums.ActionEnum.Edit%>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>&url=page|<%=page%>-category_id|<%=category_id%>-status|<%=status%>-keywords|<%=keywords%>"><%#Eval("title")%></a>(<a href="/shop/show.aspx?id=<%#Eval("id")%>" target="_blank">預覽</a>)</span></h1>
          <div class="remark">
          ...
          <div class="foot">
            ...
            <a href="tuan_edit.aspx?action=<%#TWEnums.ActionEnum.Edit%>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>&url=page|<%=page%>-category_id|<%=category_id%>-status|<%=status%>-keywords|<%=keywords%>" title="編輯" class="edit">編輯</a> <asp:LinkButton ID="lbtnCopy"   style="display:none;" CommandName="lbtnCopy" runat="server" Text="複製" />
          </div>
        </div>
      </li>
  </ItemTemplate>
